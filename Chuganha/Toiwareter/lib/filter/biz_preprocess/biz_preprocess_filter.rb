  ...
  module BizPreprocess
    class BizPreprocessFilter
      ...
      def filter(controller)
        ...
        access_cache_load
# 権限情報表示（デバッグコード）
Rails.logger.debug('BizPreprocessFilter Authority:' + controller.session[:authority_hash].to_s)
      end
      ...
      def access_cache_load
        ...
        Rails.logger.error('Exception:' + ex.class.name)
        Rails.logger.error('Message  :' + ex.message)
        Rails.logger.error('Backtrace:' + ex.backtrace)
        raise ex
      end
    end
  end
