...
module UnitTestUtil
  ...
  def error_log(message, ex=nil)
    ...
    if Exception === ex then
      ex.backtrace.each do |line|Rails.logger.error(line)end
    elsif ActiveModel::Errors === ex then
  ...
  # レコードエラーログメッセージ出力
  def record_err_log(record)
    record.errors.each do |attr, msg|
#      Rails.logger.error('Record Error ' + record.class.human_attribute_name(attr) + '=>' + msg)
      Rails.logger.error('Record Error{' + attr.to_s + '=>' + msg + '}')
    end
  end
end