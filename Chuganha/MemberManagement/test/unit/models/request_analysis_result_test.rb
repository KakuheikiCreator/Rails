...
class RequestAnalysisResultTest < ActiveSupport::TestCase
  ...
  test "2-1:Basic functions Test" do
    ...
    begin
      ...
      info.login_id = generate_str(CHAR_SET_ZENKAKU, 255)
  ...
  # バリデーションテスト
  # バリデーションチェックのテストを行う
  test "2-12:Validation Test::login_id" do
    # 数値チェック
    begin
      # 正常ケース
      info = RequestAnalysisResult.new
      ...
      info.login_id = generate_str(CHAR_SET_NUMERIC, 10)
      assert(info.valid?, "CASE:2-12-1")
      info.login_id = generate_str(CHAR_SET_ALPHANUMERIC, 255)
      assert(info.valid?, "CASE:2-12-1")
      info.login_id = "098abcd000"
      assert(info.valid?, "CASE:2-12-1")
    rescue => ex
    ...
    begin
      ...
      info.system_id = 1
      info.request_analysis_schedule_id = 1
      info.login_id = generate_str(CHAR_SET_ALPHANUMERIC, 256)
      info.request_count = 1
      assert(!info.valid?, "CASE:2-12-1")
    rescue => ex
      error_log("CASE:2-12-1 Exception:" + ex.class.name)
