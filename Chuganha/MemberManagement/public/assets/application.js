...
// 別画面起動処理
function submit_new_window(selector, target){
	var form_elms = $(selector).parents('form');
	if (form_elms.size() > 0) {
		var data_map = {};
		form_elms.find(':input').each(function() {
			...
			if (elm_name == 'screen_transition_pattern') {
				data_map[elm_name] = 2; // 新画面起動
			} else {
				data_map[elm_name] = $(this).val();
			}
		});
		var open_window = window.open("about:blank", null);
		open_window.document.open('text/html');
		open_window.document.title = "Loading...";
		open_window.document.close();
		var resp_func = function(resp_data) {
			open_window.document.open('text/html');
			open_window.document.write(resp_data);
			open_window.document.close();
		}
		// ウィンドウOPEN
		if (form_elms.attr("method") == 'post') {
			$.post(form_elms.attr("action"), data_map, resp_func);
		} else {
			$.get(form_elms.attr("action"), data_map, resp_func);
		}
	}
}
// フォームクリア処理
