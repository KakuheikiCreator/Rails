...
// Ajaxリクエスト送信
function ajax_send(form_selector, success_function){
	var form_elms = $(form_selector);
	if (form_elms.size() == 0) {
		return;
	}
	var csrf_token = $("*[name=authenticity_token]").val();
	var data_map = {'authenticity_token': csrf_token};
	form_elms.find(':input').each(function() {
		var elm_name = $(this).attr('name');
		if (elm_name == 'screen_transition_pattern') {
			data_map[elm_name] = 3; // 現在機能内遷移
		} else {
			data_map[elm_name] = $(this).val();
		}
	});
	ajax_params = {type: form_elms.attr("method"),
					url: form_elms.attr("action"),
					data: data_map,
...
// フォームコントローラー制御
//#############################################################################
// フォームのアクションを切り替えるハンドラー設定
function set_action_handler(){
	$('[action_url]').click(function(){
		if($(this).attr('action_url') == ""){return;}
		$(this).closest("form").attr('action', $(this).attr('action_url'));
	});
}
// フォーム送信処理
...
// 別画面起動処理
function submit_new_window(selector){
	var form_elms = $(selector);
	if (form_elms.is('form') != true) {
		form_elms = form_elms.parents('form');
	}
	clone_form_elms = form_elms.clone();
	clone_form_elms.attr('target', '_blank');
	clone_form_elms.find('input[type="submit"]').remove();
	clone_form_elms.find('input[name="screen_transition_pattern"]').val(2);
	clone_form_elms.hide();
	form_elms.after(clone_form_elms);
	clone_form_elms.submit();
	clone_form_elms.remove();
}
// フォームクリア処理
...
//#############################################################################
// セッション履歴クリアフラグ
var history_clr_flg = true;
// 履歴セッションクリア処理
function history_clear(action_url){
	target_form = $('#function_transition_no').parents('form').slice(0);
	if (target_form.size() <= 0) {return;}
	var data_map = {};
	data_map['screen_transition_pattern'] = 9; // 履歴クリア
	data_map['function_transition_no'] = target_form.find('#function_transition_no').val();
	data_map['synchronous_token'] = target_form.find('#synchronous_token').val();
	// 履歴セッションクリアリクエスト
	$.post(action_url, data_map);
}
// 画面アンロード前処理
function set_unload_handler(){
	$('a[target!="_blank"]').on('click', function() {history_clr_flg = false;});
	$('form[target!="_blank"]').bind('submit', function() {history_clr_flg = false;});
	$(window).unload(function(){
		if (history_clr_flg) {history_clear('/common/session/clear');}
	});
}
//#############################################################################
// コンテキストメニュー関係
