...
###############################################################################
# 業務アクションクラス
# 概要：受信したリクエスト情報をラッピングし、検証と規制リファラー情報の更新通知を行う
...
module BizActions
  module RegulationReferrerList
    class NotifyAction < BizActions::RegulationReferrerList::BaseAction
      include BizCommon
      include DataCache
      #########################################################################
      # 定数定義
      ...
      def notify?
        ...
        if valid? then
          # 更新日時更新
          if @cache.data_update?(:regulation_referrer, Time.now, true) then
            # キャッシュデータ更新通知
