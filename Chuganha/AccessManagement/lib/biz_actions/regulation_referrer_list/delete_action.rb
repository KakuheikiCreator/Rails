...
###############################################################################
# 業務アクションクラス
# 概要：受信したリクエスト情報をラッピングし、検証と規制リファラー情報の削除を行う
...
module BizActions
  module RegulationReferrerList
    class DeleteAction < BizActions::RegulationReferrerList::BaseAction
      #########################################################################
      # コンストラクタ
      #########################################################################
      ...
      def db_related_chk?
        check_result = true
        # データの有無判定
        RegulationReferrer.exists?(:id=>@delete_id.to_i)
        @delete_data = RegulationReferrer.where(:id=>@delete_id.to_i)[0]
