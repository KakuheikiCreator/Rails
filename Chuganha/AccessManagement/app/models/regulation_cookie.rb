...
class RegulationCookie < AccessManagementSuper
  ...
  # 更新項目設定
  #############################################################################
  attr_accessible :system_id, :cookie, :remarks
  #############################################################################
  # リレーション設定
  #############################################################################
  # システム
  ...
  def search_condition
    ...
    # クッキー
    where_clause.p_match(:cookie=>cookie) unless blank?(cookie)
    # 備考
  ...
  def generate_duplicate_condition
    return {:system_id => system_id, :cookie => cookie}
  end
end
