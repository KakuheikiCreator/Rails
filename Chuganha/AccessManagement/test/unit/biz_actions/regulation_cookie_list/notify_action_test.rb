...
#      print_log('system_id:' + notify_action.reg_cookie.system_id.to_s)
      assert(notify_action.reg_cookie.system_id == system_id.to_i, "CASE:2-1-1-3")
...
#      print_log('system_id:' + regulation_cookie.system_id.to_s)
      ...
      assert(notify_action.sort_item == 'system_id', "CASE:2-1-3-6")
...
#      print_log('system_id:' + notify_action.error_msg_hash[:system_id].to_s)
      assert(notify_action.error_msg_hash[:system_id] == 'システム名 は不正な値です。', "CASE:2-1-3-10")
    rescue => ex
      error_log("Exception:" + ex.class.name)
      error_log("Message  :" + ex.message)
      error_log("Backtrace:" + ex.backtrace.join("\n"))
...
#      print_log('error msg:' + notify_action.error_msg_hash.to_s)
      ...
      # アクセサ
#      print_log('system_id:' + notify_action.reg_cookie.system_id.to_s)
      assert(notify_action.reg_cookie.system_id == system_id.to_i, "CASE:2-2-1-3")
      ...
      assert(reg_cookie_list.size == 4, "CASE:2-2-1-9")
      # エラーメッセージ
#      print_log('error hash:' + notify_action.error_msg_hash.to_s)
      assert(notify_action.error_msg_hash.size == 0, "CASE:2-2-1-10")
      # 更新日時確認
#      updated_at_hash[:regulation_cookie]
#      print_log('object_id:' + updated_at_hash.wrap_obj.object_id.to_s)
#      print_log('regulation_cookie:' + updated_at_hash[:regulation_cookie].to_s)
#      assert(updated_at_hash[:regulation_cookie] > bef_time, "CASE:2-2-1-11")
    rescue => ex
      error_log("Exception:" + ex.class.name)
      error_log("Message  :" + ex.message)
      error_log("Backtrace:" + ex.backtrace.join("\n"))
