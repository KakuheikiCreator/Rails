...
#    end
    infos = RequestAnalysisSchedule.where(:gets_start_date => '2011-08-18 17:30:00 +0900')
...
#    print_log('infos.size:' + infos.size.to_s)
    ...
    begin
      info = infos[0]
      info.gets_start_date = '2011-08-18 17:40:00'
      assert(info.save!, "CASE:2-1-3-1")
      infos = RequestAnalysisSchedule.where(:gets_start_date => '2011-08-18 17:30:00')
      assert(infos.length == 0, "CASE:2-1-3-2")
      infos = RequestAnalysisSchedule.where(:gets_start_date => '2011-08-18 17:40:00')
    ...
    # 削除
    begin
      result = RequestAnalysisSchedule.where(:gets_start_date => '2011-08-18 17:40:00').destroy_all
      assert(result.length == 1, "CASE:2-1-4-1")
      result = RequestAnalysisSchedule.where(:gets_start_date => '2011-08-18 17:40:00')
...
#      print_log('result_list:' + result_list.size.to_s)
    ...
    begin
      cond_hash = Hash.new
      cond_hash[:to_datetime] = DateTime.new(2011,9,8,2,0,3)
      cond_list = RequestAnalysisSchedule.find_condition_list(cond_hash)
      assert(cond_list.size == 2, "CASE:2-5-4-1")
      assert(cond_list[0] == 'gets_start_date <= ?', "CASE:2-5-4-2")
      assert(cond_list[1] == DateTime.new(2011,9,8,2,0,3), "CASE:2-5-4-3")
      result_list = RequestAnalysisSchedule.find_list(cond_hash)
      print_log('result_list:' + result_list.size.to_s)
    ...
    begin
      cond_hash = Hash.new
      cond_hash[:system_id] = 1
      cond_hash[:from_datetime] = DateTime.new(2011,9,8,2,0,1)
      cond_hash[:to_datetime] = DateTime.new(2011,9,8,2,0,2)
      ...
      assert(cond_list[2] == DateTime.new(2011,9,8,2,0,1), "CASE:2-5-7-4")
      assert(cond_list[3] == DateTime.new(2011,9,8,2,0,2), "CASE:2-5-7-5")
      result_list = RequestAnalysisSchedule.find_list(cond_hash)
