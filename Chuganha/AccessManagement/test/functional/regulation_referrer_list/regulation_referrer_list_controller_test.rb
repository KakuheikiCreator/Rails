...
###############################################################################
# ファンクショナルテスト
# テスト対象：規制リファラー一覧コントローラー
# Copyright:: Copyright (c) 2011 仲観派
# 作成日:2012/09/04 Nakanohito
# 更新日:
###############################################################################
require 'drb/drb'
require 'test_helper'
...
class RegulationReferrerList::RegulationReferrerListControllerTest < ActionController::TestCase
...
#      print_log('@request:' + @request.request_parameters.to_s)
      assert(params[:regulation_referrer].nil?, 'CASE:2-01-01-01')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 2, 1, false, 'CASE:2-01-01-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-01-01-13')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-01-01-14')
      # ソート項目コンボ
      ...
      selector = '#input_form[action="/regulation_referrer_list/regulation_referrer_list/list"]'
      ...
      #########################################################################
      # 画面遷移パラメータチェック
      selector = '#delete_form[action="/regulation_referrer_list/regulation_referrer_list/delete"]'
      ...
      #########################################################################
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 4, 'CASE:2-01-01-24')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
    ...
    begin
      ...
      params[:regulation_referrer] = {:system_id=>'1', :referrer=>'^www\.msn\.co\.jp$', :remarks=>'MyString'}
...
#      print_log('@request:' + @request.request_parameters.to_s)
      reg_referrer = req_params[:regulation_referrer]
      assert(reg_referrer[:system_id] == '1', 'CASE:2-01-02-01')
      assert(reg_referrer[:referrer] == '^www\.msn\.co\.jp$', 'CASE:2-01-02-02')
      assert(reg_referrer[:remarks] == 'MyString', 'CASE:2-01-02-03')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 2, 1, false, 'CASE:2-01-02-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks[value="MyString"]', true, 'CASE:2-01-02-13')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer[value="^www\.msn\.co\.jp$"]', true, 'CASE:2-01-02-14')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 1, 'CASE:2-01-02-24')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
    ...
    begin
      # 送信パラメータ生成
      params = create_params(:TRANS_PTN_NOW, 2)
      params[:regulation_referrer] = {:system_id=>'a', :referrer=>'^www\.msn\.co\.jp$', :remarks=>'MyString'}
      params[:sort_item] = 'remarks'
      params[:sort_order] = 'DESC'
      params[:disp_counts] = '10'
      # 処理実行
...
#      print_log('@request:' + @request.request_parameters.to_s)
      reg_referrer = req_params[:regulation_referrer]
      assert(reg_referrer[:system_id] == 'a', 'CASE:2-01-03-01')
      assert(reg_referrer[:referrer] == '^www\.msn\.co\.jp$', 'CASE:2-01-03-02')
      assert(reg_referrer[:remarks] == 'MyString', 'CASE:2-01-03-03')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 2, 1, false, 'CASE:2-01-03-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks[value="MyString"]', true, 'CASE:2-01-03-14')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer[value="^www\.msn\.co\.jp$"]', true, 'CASE:2-01-03-15')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 4, 'CASE:2-01-03-25')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#      params = create_params(:TRANS_PTN_NEW, 1)
      ...
      params[:regulation_referrer] = {:system_id=>'1', :referrer=>'test data 1', :remarks=>'remarks 1'}
...
#      print_log('@request:' + @request.request_parameters.to_s)
      reg_referrer = req_params[:regulation_referrer]
      assert(reg_referrer[:system_id] == '1', 'CASE:2-02-01-01')
      assert(reg_referrer[:referrer] == 'test data 1', 'CASE:2-02-01-02')
      assert(reg_referrer[:remarks] == 'remarks 1', 'CASE:2-02-01-03')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 1, nil, true, 'CASE:2-02-01-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-02-01-13')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-02-01-14')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 5, 'CASE:2-02-01-24')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#      params = create_params(:TRANS_PTN_NEW, 1)
      params = create_params(:TRANS_PTN_NOW, 1)
      params[:regulation_referrer] = {:system_id=>'1', :referrer=>'^[', :remarks=>'remarks 2'}
      params[:sort_item] = 'system_id'
      params[:sort_order] = 'ASC'
      params[:disp_counts] = '500'
      post(:create, params)
...
#      print_log('@request:' + @request.request_parameters.to_s)
      reg_referrer = req_params[:regulation_referrer]
      assert(reg_referrer[:system_id] == '1', 'CASE:2-02-02-01')
      assert(reg_referrer[:referrer] == '^[', 'CASE:2-02-02-02')
      assert(reg_referrer[:remarks] == 'remarks 2', 'CASE:2-02-02-03')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 1, nil, true, 'CASE:2-02-02-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-02-02-13')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-02-02-14')
      assert_select('#sverr_referrer[value="リファラー は不正な値です。"]', true, 'CASE:2-02-02-15')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 5, 'CASE:2-02-02-25')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 1, nil, true, 'CASE:2-03-01-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-03-01-13')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-03-01-14')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 3, 'CASE:2-03-01-24')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 1, nil, true, 'CASE:2-03-02-04')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-03-02-09')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-03-02-10')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 3, 'CASE:2-03-02-21')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#      print_log('@request:' + @request.request_parameters.to_s)
      reg_referrer = req_params[:regulation_referrer]
      assert(reg_referrer[:system_id] == '1', 'CASE:2-04-01-01')
      assert(reg_referrer[:referrer] == 'test data 1', 'CASE:2-04-01-02')
      assert(reg_referrer[:remarks] == 'remarks 1', 'CASE:2-04-01-03')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 1, nil, true, 'CASE:2-04-01-09')
      # エラーメッセージ判定
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-04-01-13')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-04-01-14')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 4, 'CASE:2-04-01-24')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#      params = create_params(:TRANS_PTN_NEW, 1)
      params = create_params(:TRANS_PTN_NOW, 1)
      params[:regulation_referrer] = {:system_id=>'a', :referrer=>'^[', :remarks=>'remarks 2'}
      params[:sort_item] = 'system_id'
      params[:sort_order] = 'ASC'
      params[:disp_counts] = '500'
      post(:notify, params)
...
#      print_log('@request:' + @request.request_parameters.to_s)
      reg_referrer = req_params[:regulation_referrer]
      assert(reg_referrer[:system_id] == 'a', 'CASE:2-04-02-01')
      assert(reg_referrer[:referrer] == '^[', 'CASE:2-04-02-04')
      assert(reg_referrer[:remarks] == 'remarks 2', 'CASE:2-04-02-03')
...
#      print_log('cntr_path:' + state.cntr_path.to_s)
      ...
      valid_state('regulation_referrer_list/regulation_referrer_list', 1, nil, true, 'CASE:2-04-02-09')
      # エラーメッセージ判定
...
#      print_log('err_hash:' + err_hash.to_s)
      ...
      # 備考テキストボックス
      assert_select('#regulation_referrer_remarks', true, 'CASE:2-04-02-14')
      # リファラーテキストボックス
      assert_select('#regulation_referrer_referrer', true, 'CASE:2-04-02-15')
      # ソート項目コンボ
      ...
      # 明細件数
      reg_referrer_list = assigns[:reg_referrer_list]
      assert(reg_referrer_list.size == 4, 'CASE:2-04-02-25')
      # 明細チェック
      idx = 1
      reg_referrer_list.each do |ent|
...
#    combo_hash['選択してください。'] = ''
    ...
    valid_combo('#regulation_referrer_system_id', combo_hash, err_msg, selected_val)
  end
  ...
  def valid_details(idx, ent, err_msg)
    ...
    assert_select(head + '>td:nth-of-type(3) font', ent.referrer, err_msg)
