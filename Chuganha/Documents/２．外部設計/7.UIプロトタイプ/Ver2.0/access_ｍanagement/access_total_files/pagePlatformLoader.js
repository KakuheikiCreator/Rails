    ...
    try {
        ...
        if (myUrl) {
            ...
            if (typeof window[urlParams.namespace] !== "undefined") {
                ...
                helper.loadScript(appNameSpace.def.widgetsDomain + "/domains/blacklist.js", "mpvBlackList", window, function () {
                
                    var blackListedGlobally = mpvBlackList.global && mpvBlackList.global.test(document.domain);
                    var blackListedPrdct = mpvBlackList[appNameSpace.def.prdct] && mpvBlackList[appNameSpace.def.prdct].test(document.domain);

                    // check against global mm blacklist.
                    if (!(blackListedGlobally || blackListedPrdct)) {

                        helper.loadScript(appNameSpace.def.widgetsDomain + "/" + appNameSpace.def.version + "/misc/mntrQuery.js", "mntrjQuery", window, function () {
                            $ = mntrjQuery;
                            helper.loadScript(appNameSpace.def.widgetsDomain + "/3rdparty/json2.min.js", "JSON", window, function () {
